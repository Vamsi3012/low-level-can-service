...
openxc_DynamicField decoder_t::decode_custom(can_signal_t& signal, float value, bool* send)
{
	unsigned short encoded = value;
	unsigned short decoded = encoded << 8 | encoded >> 8;
	decoded /= 2; 
	openxc_DynamicField decoded_value = build_DynamicField((float)decoded);
